...
- A3. Peer device: Any device running Pear (app or CLI) on the local network. Discovers other peers and syncs automatically
...
- F1. Human note-taking
  ...
  - **Actors:** A1, A3
  - **Steps:** User opens Pear → navigates via tag sidebar or smart filter → creates/selects a note → writes GFM Markdown → note persists locally → syncs to any discovered peers
...
- F3. Peer discovery and sync
  - **Trigger:** A device running Pear joins a network where other Pear peers are present
  - **Actors:** A3
...
- AE2. **Covers R9, R10.** Given a user who edits three notes on their phone while away from home, when they return and their phone joins the home network where a Mac is running Pear, then all three notes sync automatically without the user taking any action
...
- AE4. **Covers R13.** Given a new user installing Pear, when they sign in with their existing ATProto handle (e.g., `@alice.bsky.social`), then their identity is established across all their devices without generating or managing cryptographic keys
- AE5. **Covers R16.** Given a Mac Mini running only the Pear CLI (no Xcode, no SwiftUI), when an AI agent creates a note via the CLI, then the note syncs to other peers on the network identically to notes created in the app
...
- AE7. **Covers R22.** Given a user who writes a skill note tagged `#skills` on their iPhone, when their Mac Mini syncs the note, then running `pear setup-skills` symlinks that note into the agent's skill directory and the agent can invoke it in subsequent sessions
...
- A user can install Pear on their Mac and iPhone, write notes on both, and see edits appear on the other device within seconds — with zero configuration beyond installing the app
...
- Cloud-hosted sync service — Pear is local-first and P2P by design, not a cloud app with offline support
...
- Web client — Pear is a native app, not a browser-based tool
- WYSIWYG rich-text editing — Pear is a Markdown editor with inline rendering, not a word processor
- Knowledge graph, backlinks, or Zettelkasten features — Pear is a notes app, not a knowledge management system
...
- **Skills as notes:** Agent skills stored as Pear notes become a P2P-synced skill distribution mechanism. Write a skill on any device, tag it `#skills`, and it propagates to all peers. A setup command bridges Pear notes into agent skill directories — making the notes app the canonical source for self-authored agent capabilities
...
- UniFFI (or similar) provides a viable Rust↔Swift bridge for the data and call patterns Pear needs
...
- [Affects R9][Resolved] Connection model: one NWConnection per peer, multiplexed document sync. Application protocol uses framed messages tagged with document ID. Discover via NWBrowser (Bonjour service type `_pear._tcp`), accept via NWListener
...
- [Affects R16][Resolved] CLI UX: verb-noun commands (`pear list`, `pear show <id>`, `pear create`, `pear edit <id>`, `pear search <query>`, `pear tags`, `pear sync-status`, `pear setup-skills`). Default output is human-readable text; `--json` flag for structured agent output. Accept content via stdin. CLI and MCP tools map 1:1
