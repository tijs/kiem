    ...
    let notes: Vec<NoteDoc> = (0..10)
        .map(|i| {
            note(
                &format!("n{i}"),
                "# N",
                &format!("2026-06-12T10:00:{i:02}Z"),
            )
        })
        .collect();
...
fn rebuild_restores_a_wiped_index() {
    ...
    assert!(
        store.search("okapi", 10).unwrap().is_empty(),
        "deleted notes stay out"
    );
}
...
fn note_types_are_settable_and_filterable() {
    ...
    let plan = store
        ...
        .unwrap();
    store
        .create_note_with_type("# Decision\n\n#proj/x", DID, "decision")
        .unwrap();
...
fn filter_counts_match_the_list_queries() {
    let mut store = NoteStore::open_in_memory().unwrap();
    store
        .create_note("# Todo note\n\n- [ ] open item #x", DID)
        .unwrap(); // todo + today + tagged
    store
        .create_note("# Untagged, modified today", DID)
        .unwrap();
    ...
    assert_eq!(
        counts.untagged as usize,
        store.list_untagged().unwrap().len()
    );
